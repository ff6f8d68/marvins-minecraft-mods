...
// Inlined Perlin noise (from noise.glsl) to avoid #moj_import namespace resolution issue
...
float cnoise(vec3 P) {
    ...
    vec3 fade_xyz = vec3(0.0); // simplified fade for warp - we use basic interpolation
    fade_xyz = Pf0*Pf0*Pf0*(Pf0*(Pf0*6.0-15.0)+10.0);
